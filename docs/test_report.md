...
聊天应用 Tool 与任务清单测试额外覆盖：

- 目标、任务查询 Tool 的结构化结果。
- 创建目标、向目标添加任务和重复任务去重。
- 按任务 ID 标记完成或重新打开。
- 操作型聊天触发 Tool，普通聊天继续走模型回复。
- Tool 计划失败时不伪造“已执行”结果。
- 今天/本周个人任务的数据库迁移、查询与完成状态。
...
已对运行中的 `127.0.0.1:8765` 完成健康检查、Dashboard、日报、备份、
静态资源和错误输入联调，前端请求路径与后端路由一致。但当前 pytest 套件
尚未直接启动临时 HTTP 服务验证全部 `/api/*` 状态码和响应 Schema。

后续应增加隔离数据库下的 Web handler 集成测试，避免接口字段调整时只在
浏览器运行阶段发现问题。
...
2. 增加 `/api/*` 路由、状态码和响应 Schema 的 HTTP 集成测试。
3. 为 PDF 转换建立独立适配器测试。
